...
To check everything is set up, try the TestLevel Unreal engine map. BP_PlayerState contains Websocket & NanoManager components. NanoManager has the required http server details which need changing. A video demonstration is coming soon.

Implementation details:  
`NanoBlueprintLibrary` contains various generic functions such as creating seeds, encrypting/decrypting them using AES, converting to accounts, converting between Raw and Nano and various other things.  
